...
from .forms import LoginForm, CreateAccountForm, CreatePostAnyGroupForm
...
def format_local_time(utc_timestamp):
	...
	if local_timestamp.year == datetime.now(LOCAL_TIMEZONE).year:
		difference = datetime.now(LOCAL_TIMEZONE) - local_timestamp
		if difference < timedelta(days=7):
			return "{} days ago at {dt:%I}:{dt:%M} {dt:%p}".format(difference.days, dt=local_timestamp)
		else:
			return "at {dt:%I}:{dt:%M} {dt:%p}, {dt:%A} {dt.day} {dt:%B}".format(dt=local_timestamp)
	else:
		return "at {dt:%I}:{dt:%M} {dt:%p}, {dt:%A}, {dt.day} {dt:%B} {dt:%Y}".format(local_timestamp)
